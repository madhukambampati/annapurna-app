...
export function rulesList(s: Settings): string[] {
  ...
  return [
    ...
    "For custom/catering orders, never say the order is confirmed just because a price was discussed. Once Annapurna Home Foods has quoted the final price and the pickup day/time is known, the customer's explicit confirmation is what places the order. The system code creates the confirmation card and order record. Until those terms are complete, say the request is saved and Annapurna Home Foods will finalize the remaining details here.",
